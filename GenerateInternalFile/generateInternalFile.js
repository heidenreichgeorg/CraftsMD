...
function internHazard(hazardName) {
  let entry=null;
    if(entry=findByKey(jRiskFile.regHazard,"name",hazardName)) {} else  {
      entry = {"id":"HAZ"+next(),  'title':'Hazard',  "name":hazardName};
      jRiskFile.regHazard.push(entry);
    }
    return entry;
}
...
function addRawRIT2InternalFile(jCORI) {
  ...
  let strHazard = jCORI.hazard.name;
  let jHazard = internHazard(strHazard);
  console.log("DSH: "+JSON.stringify(jHazard)); // { id name }
  ...
  let targetRisk =findByKey(jRiskFile.regControlledRisk,"hazard",jHazard.name);
  // 20230708 let targetRisk =findByKey(jRiskFile.relDomainSpecificHazard,"harm",jHarm.name);
  if(targetRisk) {
    console.log("DSH found:  "+JSON.stringify(jHarm.name)+"   "+JSON.stringify(jHazard.name));
  }
  else {
    targetRisk = { 
        'id':'RIT'+next(), 
